...
- **发布链冻结包形态分支**：`publish` / `upload` 只接受源码形态（目录含 `release.json`），缺失即报错并提示先创建 `release.json`；不再支持「本地 skill.json 包直接推成源码」的旧路径。`publish` 在缺失 `release.json` 时自动补最小清单（`schemaVersion: 1`，`license` 由用户显式确认——它无法推断，不设默认猜测；`keywords` / `compatibility` / `dependencies` 为空集合），落盘到源码目录，因为服务器发布时以源码 commit 的 `release.json` 为准（`skills.ts:210-216`），仅内存生成会被忽略。
