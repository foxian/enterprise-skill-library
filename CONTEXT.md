...
所有者成员）的统称。三种身份都由**常设团队成员身份推导**，组织内不为它们存储
任何角色。组织成员身份不是平台角色（见平台角色）。

## 普通成员 (Ordinary Member)

组织成员之一，既不属于其组织管理团队，也不属于其组织技能管理团队。加入组织时
自动加入组织只读、读写两个常设团队，离开时自动移出；对组织内 private 技能默认
没有任何权限（ADR-0032）。
_Avoid_: 普通用户。

## 管理成员 (Managing Member)

组织成员之一，即该组织**技能管理团队**（`all-managers`）的成员。持有本组织技能
的管理权；该权限逐技能授予，默认不含任何技能（见技能可见性）。

## 所有者成员 (Owner Member)

组织成员之一，即该组织**组织管理团队**（Owners）的成员。持有该组织最高管理权，
可变更其他成员的身份（见组织管理团队）。组织必须至少保留一名所有者成员。
...
变更其他成员的身份、管理团队与组织设置，可见并管理本组织名下全部技能，并可
删除组织（ADR-0033、ADR-0034）。**身份变更（提为管理成员 / 所有者成员，或收回
管理成员身份）只有本团队能做**——管理成员不碰身份。成员之间可以互相移出组织
——不能移除自己，且团队必须至少保留一名成员；ESL Platform Administrator 不经
成员身份也有同款能力（含添加成员），但同样不破"至少保留一名"这条不变量——
无主组织不是可治理状态，确实无人可用的组织走整体删除。
...
团队由所有者成员管理。每个组织创建时自动生成四个常设团队：组织只读
团队（Read）、组织读写团队（Write）、组织技能管理团队（Manage）与组织管理
团队（Owners，见其词条）；成员加入组织时自动加入组织只读、读写两个常设团队，
离开组织时自动移出——技能管理团队**不**自动加入，它承载**管理成员**身份（见其
词条）。常设团队不可删除、不可改名；自定义团队可创建、改名、删除（ADR-0032）。
